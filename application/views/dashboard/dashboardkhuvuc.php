...
<script>
    ...
    function update_data_chart(chart){
        ...
        $.post(url,{
            ...
            switch(response.state){
                case 'success':{
                    $('#bieuDoTong .card-title').html("Sản lượng: " + $( ".search_khuvuc option:selected" ).text());
    ...
    function renderCardThietbi(){
        var user = $('.search_taiKhoanKhachHang').val();
        var idKhuVuc = $('.search_khuvuc').val();
        let url = base_url+'/Dashboard/get_EnergyCurrentOfDevice';
        $.post(url,{
            'user': user,
            'idKhuVuc': idKhuVuc,
        },function(response){
            switch(response.state){
                case 'success':
                    let listThietBi = response.data;
                    let htmlListThietBi = "";
                    let headerTongPAC = 0;
                    let PACHeader_donvi="";
                    for(let i=0;i<listThietBi.length;i++){
                        let thietbi = listThietBi[i];
                        if(thietbi.PAC.Value!=""){
                            
                            if(thietbi.PAC.Unit=='kW'){
                                headerTongPAC += parseFloat(thietbi.PAC.Value);
                            }else{
                                headerTongPAC += (parseFloat(thietbi.PAC.Value)/1000);
                            }
                        }
                        
                        let tenTrangThai = thietbi.Online==1? '<span class="color_red">Online</span>' : '<span class="color_red">Offline</span>';
                        let htmlCardThietBi = '<div class="form-group col-sm-4" >'+
                        ...
                        '                <div class="c-vr"></div> '+
                        '                <div class="col"> '+
                        '                    <div class="text-value-lg">'+ formatNumber(thietbi.month.Value) + ' '+ formatDonVi(thietbi.month.Unit) +'</div> '+
                        '                    <div class="text-uppercase text-muted small">Sản lượng tháng</div> '+
                        '                    <div class="text-value-lg">$'+ thietbi.month.tien + ' VNĐ</div> '+
                        ...
                        '                <div class="c-vr"></div> '+
                        '                <div class="col"> '+
                        '                   <button class="btn btn-info-theme btn-xemthietbi" idThietBi='+thietbi.idThietBi+'>Chi tiết thiết bị </button>' +
                        '                </div> '+
                        '            </div> '+
                        '       </div> '+
                        '    </div> '+
                        ...
                        htmlListThietBi+=htmlCardThietBi;
                    }
                    $('#div-card-thietbi').html(htmlListThietBi);
                    $('.btn-xemthietbi').on('click',function(){
                        let idThietBi = $(this).attr('idThietBi');
                        // console.log(idKhuVuc);
                        window.location.href = base_url+'/Dashboard/thietbi/'+user+'/'+idKhuVuc+'/'+idThietBi+"/";
                    });
                    ...
                    $('.c-header-nav .dataDong1').html('PAC khu vực: '+ formatNumber(headerTongPAC) +' '+ formatDonVi(PACHeader_donvi));
                break;
                default: 
                break;
            }
    ...
    function update_data_chartPAC(chart){
        ...
        $.post(url,{
            ...
            switch(response.state){
                case 'success':{
                    $('#bieuDoPAC .card-title').html("PAC: " + $( ".search_khuvuc option:selected" ).text());
